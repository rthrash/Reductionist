 ...
 */
final class Imagine implements ImagineInterface, RImagineInterface
{
	/**
	...
	public function open($path) {
		try {
			...
			$image = new Image($imagick, $this->createPalette($imagick));
		}
		catch (\Exception $e) {
	...
	public function Ropenjpg($path, BoxInterface $size) {
		try {
			$imagick = new \Imagick();
			$imagick->setOption('jpeg:size', "{$size->getWidth()}x{$size->getHeight()}");
